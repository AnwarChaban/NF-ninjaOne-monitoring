...
exports.fetchNinjaOneBackups = fetchNinjaOneBackups;
exports.syncNinjaOneCustomers = syncNinjaOneCustomers;
exports.syncNinjaOneDevices = syncNinjaOneDevices;
exports.syncNinjaOneData = syncNinjaOneData;
exports.syncNinjaOneUsers = syncNinjaOneUsers;
...
const PRODUCT_VERSION_FIELD_MAP = {
...
function getCustomField(device, keys) {
...
async function fetchDeviceCustomFieldMap(apiUrl, deviceId, authorizationHeader) {
    const endpoints = [
...
function extractSoftwareEntries(device, fieldMap) {
    const entries = [];
    const seen = new Set();
    const pushEntry = (productRaw, versionRaw) => {
    ...
    for (const [key, value] of Object.entries(fieldMap)) {
        ...
        if (!suffix)
            continue;
        const version = fieldMap[`currentversion_${suffix}`] ||
        ...
        if (version)
            pushEntry(value, version);
    }
    return entries;
}
function getDevicesFromApiPayload(payload) {
...
async function fetchOrganizationDevices(apiUrl, orgId, authorizationHeader) {
    const endpoints = [
...
function saveNinjaOneCustomers(customers) {
    ...
    const upsertNinjaOneCustomer = db.prepare(`INSERT INTO ninjaone_customers (customer_id, ninja_org_id, name, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?)
    ...
    for (const customer of customers) {
        ...
        if (!orgId)
            continue;
        let customerId = selectCustomer.get(customer.name)?.id;
        if (!customerId) {
...
function saveNinjaOneDevices(customers) {
    const db = (0, db_1.getDb)();
    const now = new Date().toISOString();
    ...
    const insertNinjaDevice = db.prepare(`INSERT INTO ninjaone_devices (ninjaone_customer_id, product_id, external_device_id, name, current_version, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?)`);
    ...
    const transaction = db.transaction(() => {
        for (const customer of customers) {
            ...
            if (!orgId)
                continue;
            const ninjaRow = selectNinjaOneCustomer.get(String(orgId));
            if (!ninjaRow)
                continue;
            ...
            for (const device of customer.devices) {
                ...
                insertNinjaDevice.run(ninjaRow.id, productId, `ninja-${device.ninjaDeviceId || 'unknown'}`, device.name, device.currentVersion || '', now, now);
                deviceCount++;
            }
        }
    });
...
function getAuthBaseUrl(apiUrl) {
    if (!apiUrl)
...
async function getAuthorizationHeader(apiUrl, apiKey, clientId, clientSecret) {
    if (apiKey) {
...
async function fetchFromNinjaOne() {
    const { apiUrl, apiKey, clientId, clientSecret } = (0, runtime_settings_1.getNinjaOneRuntimeConfig)();
    if (!apiUrl) {
...
async function fetchNinjaOneBackups() {
    const { apiUrl, apiKey, clientId, clientSecret } = (0, runtime_settings_1.getNinjaOneRuntimeConfig)();
    if (!apiUrl)
        throw new Error('NinjaOne API URL is required');
    const authorizationHeader = await getAuthorizationHeader(apiUrl, apiKey, clientId, clientSecret);
...
async function syncNinjaOneCustomers(triggeredBy = 'cron') {
    if (!(0, runtime_settings_1.isNinjaOneConfigured)())
        return { customers: 0 };
    const syncId = (0, sync_history_1.startSync)('ninjaone', triggeredBy, 'ninjaone_customers');
    try {
        const data = await fetchFromNinjaOne();
        const count = saveNinjaOneCustomers(data);
        (0, sync_history_1.completeSync)(syncId, 0, count);
...
async function syncNinjaOneDevices(triggeredBy = 'cron') {
    if (!(0, runtime_settings_1.isNinjaOneConfigured)())
        return { devices: 0 };
    const syncId = (0, sync_history_1.startSync)('ninjaone', triggeredBy, 'ninjaone_devices');
    try {
        const data = await fetchFromNinjaOne();
        const count = saveNinjaOneDevices(data);
        (0, sync_history_1.completeSync)(syncId, count);
...
async function syncNinjaOneData(triggeredBy = 'cron') {
    if (!(0, runtime_settings_1.isNinjaOneConfigured)())
        return { customers: 0, devices: 0 };
    const custId = (0, sync_history_1.startSync)('ninjaone', triggeredBy, 'ninjaone_customers');
    const devId = (0, sync_history_1.startSync)('ninjaone', triggeredBy, 'ninjaone_devices');
    try {
        const data = await fetchFromNinjaOne();
        const customers = saveNinjaOneCustomers(data);
        (0, sync_history_1.completeSync)(custId, 0, customers);
        const devices = saveNinjaOneDevices(data);
        (0, sync_history_1.completeSync)(devId, devices);
...
async function syncNinjaOneUsers(_triggeredBy = 'manual') {
    if (!(0, runtime_settings_1.isNinjaOneConfigured)())
        throw new Error('NinjaOne ist nicht konfiguriert');
    const { apiUrl, apiKey, clientId, clientSecret } = (0, runtime_settings_1.getNinjaOneRuntimeConfig)();
    ...
    if (!res.ok)
        throw new Error(`NinjaOne /users Fehler: ${res.status} ${res.statusText}`);
    ...
    const updateByNinjaUid = db.prepare('UPDATE users SET display_name = ?, email = ? WHERE ninja_uid = ?');
    const linkNinjaUid = db.prepare('UPDATE users SET ninja_uid = ?, display_name = ? WHERE id = ?');
    const insertUser = db.prepare('INSERT INTO users (username, display_name, role, email, ninja_uid, created_at) VALUES (?, ?, ?, ?, ?, ?)');
    let created = 0;
    let updated = 0;
    const transaction = db.transaction(() => {
        for (const u of filtered) {
            ...
            const byUid = selectByNinjaUid.get(ninjaUid);
            if (byUid) {
            ...
            const byEmail = selectByEmail.get(email);
            if (byEmail) {
