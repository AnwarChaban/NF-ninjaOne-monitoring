...
exports.syncUnifiData = syncUnifiData;
...
function pickCustomerForHost(hostName, customers) {
    const normalizedHostName = normalizeName(hostName);
    if (!normalizedHostName)
        return { ambiguous: false };
    const strictMatches = customers.filter(customer => normalizedHostName.includes(customer.normalizedName) || customer.normalizedName.includes(normalizedHostName));
...
function pickManualCustomerForHost(hostName, mappings, customersById) {
    const normalizedHostName = normalizeName(hostName);
    if (!normalizedHostName)
        return { ambiguous: false };
...
function isGroupedHostRecord(raw) {
    return Array.isArray(raw?.devices);
}
function flattenGroupedDeviceRecords(records) {
    const flattened = [];
...
function extractGroupedHostRecords(records) {
    return records.filter(isGroupedHostRecord);
}
function extractNetworkControllerVersion(reportedState) {
    const controllers = Array.isArray(reportedState?.controllers) ? reportedState.controllers : [];
    const networkController = controllers.find((controller) => {
    ...
    return {
        currentVersion: normalizeString(networkController?.version ||
...
function withPaging(url, limit, offset) {
    const parsed = new URL(url);
    parsed.searchParams.set('limit', String(limit));
    parsed.searchParams.set('offset', String(offset));
    return parsed.toString();
}
function resolveNextUrl(baseUrl, payload) {
...
function getRecordIdentifier(item, fallbackIndex) {
    const preferredKeys = [
        item?.id,
        item?.deviceId,
        item?.device_id,
...
function getAuthCandidates(apiKey) {
...
async function fetchWithAuthFallback(url, authCandidates, entityName) {
...
async function fetchPaginatedRecords(url, authCandidates, entityName) {
    const limit = 200;
    const maxPages = 100;
...
function parseHost(raw) {
    ...
    const consoleDevice = nestedDevices.find((device) => !!device?.isConsole) || null;
    return {
        ...
        osCurrentVersion: normalizeString(reportedHardware?.firmwareVersion ||
            reported?.firmwareVersion ||
            consoleDevice?.version ||
            raw?.firmwareVersion ||
            raw?.version),
        osLatestVersion: normalizeString(firmwareUpdate?.latestAvailableVersion ||
...
function parseDevice(raw) {
    ...
    return {
        ...
        version: normalizeString(hardware?.firmwareVersion ||
            raw?.firmwareVersion ||
            raw?.firmware?.version ||
            reported?.firmwareVersion ||
            raw?.version ||
            reported?.version),
        latestAvailableVersion: normalizeString(firmwareUpdate?.latestAvailableVersion ||
            raw?.updateAvailable ||
            raw?.latestAvailableVersion ||
...
function toComparableSemver(version) {
    ...
    if (!cleaned)
        return null;
    const coerced = semver_1.default.coerce(cleaned);
...
function pickHighestVersion(versions) {
    ...
    for (const candidateRaw of versions) {
        ...
        if (candidateSemver && highestSemver) {
            if (semver_1.default.gt(candidateSemver, highestSemver)) {
                highestRaw = candidate;
                highestSemver = candidateSemver;
            }
            continue;
...
async function syncUnifiData(triggeredBy = 'cron') {
    if (!(0, runtime_settings_1.isUnifiConfigured)()) {
        throw new Error('UniFi is not configured');
    }
    const custId = (0, sync_history_1.startSync)('unifi', triggeredBy, 'unifi_customers');
    const devId = (0, sync_history_1.startSync)('unifi', triggeredBy, 'unifi_devices');
    try {
        const result = await _syncUnifiDataInternal();
        (0, sync_history_1.completeSync)(custId, 0, result.customers);
...
async function _syncUnifiDataInternal() {
    if (!(0, runtime_settings_1.isUnifiConfigured)()) {
        throw new Error('UniFi is not configured');
    }
    const runtime = (0, runtime_settings_1.getUnifiRuntimeConfig)();
    ...
    for (const hostRaw of hostCandidates) {
        ...
        if (!parsedHost.id || !parsedHost.name)
            continue;
    ...
    const customersById = new Map(normalizedCustomers.map(customer => [customer.id, customer]));
    const manualMappings = db.prepare('SELECT match_text, customer_id FROM unifi_customer_mappings').all();
    const normalizedMappings = manualMappings
        .map(mapping => ({
    ...
    const devicesByController = new Map();
    const devicesByHostId = new Map();
  ...
  `);
    ...
    const insertUnmatchedHost = db.prepare('INSERT INTO unifi_unmatched_hosts (host_id, host_name, reason, synced_at) VALUES (?, ?, ?, ?)');
  ...
  `);
    ...
    const insertedNetworkDeviceKeys = new Set();
    const osLatestCandidates = [];
    const networkLatestCandidates = [];
    const transaction = db.transaction(() => {
        const unifiCustomerIdMap = new Map();
        ...
        for (const host of hosts) {
            ...
            if (!unifiCustomerIdMap.has(customer.id)) {
                ...
                const row = selectUnifiCustomer.get(customer.id);
            ...
            if (host.osCurrentVersion) {
                ...
                insertDevice.run(unifiCustId, 'unifi-os', host.id ? `os-${host.id}` : `os-${host.name}`, `${host.name} (UniFi OS)`, host.osCurrentVersion, now, now);
                insertedDevices++;
            ...
            if (host.networkCurrentVersion) {
                ...
                insertDevice.run(unifiCustId, 'unifi-network', host.id ? `net-${host.id}` : `net-${host.name}`, `${host.name} (Network App)`, host.networkCurrentVersion, now, now);
                insertedDevices++;
                if (isCleanVersion(networkTargetVersion))
            ...
            for (const device of uniqueDevices.values()) {
                ...
                insertDevice.run(unifiCustId, 'unifi-network', `device-${device.id}`, `${device.name || `UniFi Device ${device.id}`} (${device.deviceType || 'Device'})`, version, now, now);
                insertedDevices++;
    ...
    if (highestOsVersion) {
        (0, products_1.storeProductVersion)('unifi-os', highestOsVersion, 'unifi', runtime.hostsApiUrl);
    }
    if (highestNetworkVersion) {
        (0, products_1.storeProductVersion)('unifi-network', highestNetworkVersion, 'unifi', runtime.hostsApiUrl);
    }
    return {
