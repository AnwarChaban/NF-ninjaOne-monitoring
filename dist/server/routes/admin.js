...
router.use('/admin', auth_1.requireAuth, (0, auth_1.requireRole)('administrator'));
...
function extractNinjaDeviceId(externalDeviceId) {
    if (!externalDeviceId)
        return undefined;
...
router.get('/admin/scraper-products', (_req, res) => {
    ...
    res.json(products.map(p => {
        const latest = (0, products_1.getLatestVersion)(p.id);
        return {
            product: p.id,
            name: version_fetcher_1.productNames[p.id] || p.name,
            active: p.active === 1,
...
router.put('/admin/scraper-products/:id', (req, res) => {
    const { active, name, latestVersion, releaseUrl } = req.body;
    const existing = (0, products_1.getProduct)(req.params.id);
    if (!existing) {
        res.status(404).json({ error: 'Product not found' });
        return;
    }
    (0, products_1.updateProduct)(req.params.id, {
        ...(active !== undefined && { active: active ? 1 : 0 }),
        ...(name !== undefined && { name }),
    });
    if (latestVersion) {
        (0, products_1.storeProductVersion)(req.params.id, latestVersion, 'scraped', releaseUrl);
    }
    (0, audit_1.logAction)(req.user, 'product.update', 'product', req.params.id, name ?? req.params.id, { active, latestVersion }, req);
    res.json({ ok: true });
});
// --- Custom Products ---
...
router.post('/admin/custom-products', (req, res) => {
    const { id, name, latestVersion, releaseUrl } = req.body;
    if (!id || !name || !latestVersion) {
        res.status(400).json({ error: 'id, name and latestVersion are required' });
        return;
    }
    if ((0, products_1.getProduct)(id)) {
        res.status(409).json({ error: 'Product already exists' });
        return;
    }
    (0, products_1.createProduct)(id, name, 'custom');
    (0, products_1.storeProductVersion)(id, latestVersion, 'scraped', releaseUrl);
    (0, audit_1.logAction)(req.user, 'product.create', 'product', id, name, { latestVersion }, req);
    res.json({ ok: true });
});
router.put('/admin/custom-products/:id', (req, res) => {
    const { name, latestVersion, releaseUrl, active } = req.body;
    const existing = (0, products_1.getProduct)(req.params.id);
    if (!existing) {
        res.status(404).json({ error: 'Product not found' });
        return;
    ...
    (0, audit_1.logAction)(req.user, 'product.update', 'product', req.params.id, name ?? existing.name, { active, latestVersion }, req);
    res.json({ ok: true });
});
router.delete('/admin/custom-products/:id', (req, res) => {
    ...
    (0, audit_1.logAction)(req.user, 'product.delete', 'product', req.params.id, existing.name, null, req);
    res.json({ ok: true });
});
...
router.post('/admin/products', (req, res) => {
    ...
    (0, products_1.createProduct)(id, name, type || 'custom');
    (0, audit_1.logAction)(req.user, 'product.create', 'product', id, name, { type }, req);
    res.json({ ok: true });
});
router.delete('/admin/products/:id', (req, res) => {
    ...
    const existing = (0, products_1.getProduct)(req.params.id);
    db.prepare('DELETE FROM products WHERE id = ?').run(req.params.id);
    (0, audit_1.logAction)(req.user, 'product.delete', 'product', req.params.id, existing?.name ?? req.params.id, null, req);
    res.json({ ok: true });
});
router.put('/admin/products/:id', (req, res) => {
    ...
    (0, audit_1.logAction)(req.user, 'product.update', 'product', req.params.id, name ?? existing.name, { active }, req);
    res.json({ ok: true });
});
// --- Customers ---
...
router.post('/admin/customers', (req, res) => {
    ...
    const result = db
        ...
        .run(name, now, now);
    (0, audit_1.logAction)(req.user, 'customer.create', 'customer', Number(result.lastInsertRowid), name, null, req);
...
router.put('/admin/customers/:id', (req, res) => {
    ...
    (0, audit_1.logAction)(req.user, 'customer.update', 'customer', customerId, name, null, req);
    res.json({ ok: true });
});
router.delete('/admin/customers/:id', (req, res) => {
    const db = (0, db_1.getDb)();
    const id = parseInt(req.params.id);
    const row = db.prepare('SELECT name FROM customers WHERE id = ?').get(id);
    db.prepare('DELETE FROM customers WHERE id = ?').run(id);
    (0, audit_1.logAction)(req.user, 'customer.delete', 'customer', id, row?.name ?? String(id), null, req);
    res.json({ ok: true });
});
// --- Integration Accounts ---
router.post('/admin/customers/:id/ninjaone', (req, res) => {
    const db = (0, db_1.getDb)();
    const { ninjaOrgId, name } = req.body;
    const customerId = parseInt(req.params.id);
    if (!ninjaOrgId || !name) {
    ...
    try {
        ...
        (0, audit_1.logAction)(req.user, 'integration.create', 'ninjaone', customerId, name, { ninjaOrgId }, req);
        res.json({ ok: true, id: result.lastInsertRowid });
    }
    catch {
...
router.post('/admin/customers/:id/unifi', (req, res) => {
    const db = (0, db_1.getDb)();
    const { unifiCustomerId, name } = req.body;
    const customerId = parseInt(req.params.id);
    ...
    try {
        ...
        (0, audit_1.logAction)(req.user, 'integration.create', 'unifi', customerId, name, { unifiCustomerId }, req);
        res.json({ ok: true, id: result.lastInsertRowid });
    }
    catch {
...
router.post('/admin/customers/:id/sophos', (req, res) => {
    const db = (0, db_1.getDb)();
    const { sophosCustomerId, name } = req.body;
    const customerId = parseInt(req.params.id);
    ...
    try {
        ...
        (0, audit_1.logAction)(req.user, 'integration.create', 'sophos', customerId, name, { sophosCustomerId }, req);
        res.json({ ok: true, id: result.lastInsertRowid });
    }
    catch {
...
router.post('/admin/customers/:id/devices', (req, res) => {
    const db = (0, db_1.getDb)();
    const customerId = parseInt(req.params.id);
    const { name, product, currentVersion, ninjaDeviceId } = req.body;
    ...
    if (!(0, products_1.getProduct)(product)) {
        (0, products_1.createProduct)(product, version_fetcher_1.productNames[product] || product, 'scraped');
    }
    const now = new Date().toISOString();
    ...
    let ninjaOneCustomer = db
        ...
        .get(customerId, manualOrgId);
    if (!ninjaOneCustomer) {
        const result = db
            .prepare('INSERT INTO ninjaone_customers (customer_id, ninja_org_id, name, created_at, updated_at) VALUES (?, ?, ?, ?, ?)')
            .run(customerId, manualOrgId, `Manuell`, now, now);
        ninjaOneCustomer = { id: result.lastInsertRowid };
    }
    ...
    (0, audit_1.logAction)(req.user, 'device.create', 'device', Number(result.lastInsertRowid), name, { product, currentVersion, customerId }, req);
    res.json({ ok: true, id: result.lastInsertRowid });
});
router.put('/admin/devices/:id', (req, res) => {
    const db = (0, db_1.getDb)();
    const encodedId = parseInt(req.params.id);
    const { name, product, currentVersion } = req.body;
    const now = new Date().toISOString();
    ...
    (0, audit_1.logAction)(req.user, 'device.update', 'device', encodedId, name ?? String(encodedId), { source, product, currentVersion }, req);
    res.json({ ok: true });
});
router.delete('/admin/devices/:id', (req, res) => {
    ...
    (0, audit_1.logAction)(req.user, 'device.delete', 'device', encodedId, String(encodedId), { source }, req);
    res.json({ ok: true });
});
// --- Backup Accounts ---
router.post('/admin/customers/:id/backup', (req, res) => {
    ...
    try {
        ...
        (0, audit_1.logAction)(req.user, 'backup_account.create', 'backup_account', customerId, name, { fromEmail }, req);
        res.json({ ok: true, id: result.lastInsertRowid });
    }
    catch {
...
router.delete('/admin/customers/:id/backup', (req, res) => {
    const db = (0, db_1.getDb)();
    const id = parseInt(req.params.id);
    const row = db.prepare('SELECT name FROM backup_accounts WHERE customer_id = ?').get(id);
    db.prepare('DELETE FROM backup_accounts WHERE customer_id = ?').run(id);
    (0, audit_1.logAction)(req.user, 'backup_account.delete', 'backup_account', id, row?.name ?? String(id), null, req);
    res.json({ ok: true });
});
// --- NinjaOne Sync ---
router.post('/admin/ninjaone/sync', async (req, res) => {
    ...
    try {
        const result = await (0, ninjaone_1.syncNinjaOneData)(`manual:${req.user?.username}`);
        (0, audit_1.logAction)(req.user, 'sync.manual', 'integration', 'ninjaone', 'NinjaOne', result, req);
...
router.post('/admin/unifi/sync', async (req, res) => {
    if (!(0, runtime_settings_1.isUnifiConfigured)()) {
        res.status(400).json({ error: 'UniFi is not configured' });
        return;
    }
    try {
        const result = await (0, unifi_1.syncUnifiData)(`manual:${req.user?.username}`);
        (0, audit_1.logAction)(req.user, 'sync.manual', 'integration', 'unifi', 'UniFi', result, req);
        res.json({ ok: true, ...result });
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
router.delete('/admin/customers/:id/sophos', (req, res) => {
    const db = (0, db_1.getDb)();
    const id = parseInt(req.params.id);
    const row = db.prepare('SELECT name FROM sophos_customers WHERE customer_id = ?').get(id);
    db.prepare('DELETE FROM sophos_customers WHERE customer_id = ?').run(id);
    (0, audit_1.logAction)(req.user, 'integration.delete', 'sophos', id, row?.name ?? String(id), null, req);
    res.json({ ok: true });
});
...
router.post('/admin/sophos/assign-tenant', (req, res) => {
    const db = (0, db_1.getDb)();
    const { customerId, tenantId, tenantName } = req.body;
    ...
    try {
        ...
        (0, audit_1.logAction)(req.user, 'integration.create', 'sophos', customerId, tenantName, { tenantId }, req);
        res.json({ ok: true });
    }
    catch {
...
router.get('/admin/sophos/api-tenants', async (_req, res) => {
    ...
    try {
        const tenants = await (0, sophos_1.fetchTenantsFromApi)();
        res.json(tenants);
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
router.post('/admin/sophos/sync', async (req, res) => {
    ...
    try {
        const result = await (0, sophos_1.syncSophosData)(`manual:${req.user?.username}`);
        const alertResult = await (0, sophos_1.syncSophosAlerts)().catch(err => {
            console.error('[Sophos] Alert sync failed after device sync:', err);
            return { total: 0 };
        });
        (0, audit_1.logAction)(req.user, 'sync.manual', 'integration', 'sophos', 'Sophos', { ...result, alerts: alertResult.total }, req);
...
router.post('/admin/sophos/sync-alerts', async (req, res) => {
    ...
    try {
        const result = await (0, sophos_1.syncSophosAlerts)();
        (0, audit_1.logAction)(req.user, 'sync.manual', 'integration', 'sophos_alerts', 'Sophos Alerts', result, req);
        res.json({ ok: true, ...result });
    }
    catch (error) {
        res.status(500).json({ error: error.message });
  ...
  `).all();
    ...
    `).all(uc.id);
        const devicesWithStatus = devices.map(d => {
            const latest = (0, products_1.getLatestVersion)(d.productId);
            const latestVersion = latest?.version ?? '';
            const status = latestVersion ? (0, comparator_1.compareVersions)(d.currentVersion, latestVersion, d.productId).status : 'unknown';
...
router.post('/admin/unifi/mappings', (req, res) => {
    const db = (0, db_1.getDb)();
    const { matchText, customerId } = req.body;
    ...
    try {
        ...
        (0, audit_1.logAction)(req.user, 'unifi_mapping.create', 'unifi_mapping', Number(result.lastInsertRowid), matchText, { customerId }, req);
        res.json({ ok: true, id: result.lastInsertRowid });
    }
    catch (err) {
...
router.delete('/admin/unifi/mappings/:id', (req, res) => {
    const db = (0, db_1.getDb)();
    const id = parseInt(req.params.id);
    const row = db.prepare('SELECT match_text FROM unifi_customer_mappings WHERE id = ?').get(id);
    db.prepare('DELETE FROM unifi_customer_mappings WHERE id = ?').run(id);
    (0, audit_1.logAction)(req.user, 'unifi_mapping.delete', 'unifi_mapping', id, row?.match_text ?? String(id), null, req);
    res.json({ ok: true });
});
...
router.get('/admin/ninjaone/backups', async (_req, res) => {
    try {
        const backups = await (0, ninjaone_1.fetchNinjaOneBackups)();
        res.json(backups);
    }
    catch (error) {
